...
export default function ProverbScreen({ onBack, navigate }) {
  const [question, setQuestion] = useState("Que disent les proverbes sur le travail ?");
  ...
  async function submit(q) {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/ask/proverbe`, {
        method: "POST",
        ...
        <header className="mb-12 flex items-center justify-between animate-fade-in">
          ...
          <div className="text-center">
            ...
            <p className="text-sm uppercase tracking-[0.3em] font-medium" style={{ color: 'var(--text-muted)' }}>
              Paroles précieuses des anciens
            </p>
          </div>
