...
export default function SuivreSilhouetteScreen({ navigate }) {
  return (
    <div style={{ minHeight: "100vh", background: "#1B4332", color: "#fff", display: "flex", flexDirection: "column", justifyContent: "center", padding: "2rem", position: 'relative' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1 }}>
        <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: "2.2rem", marginBottom: "1.5rem", textAlign: "center" }}>
          Tu décides de suivre la silhouette…
        </h1>
        <p style={{ fontSize: "1.1rem", maxWidth: 500, textAlign: "center", marginBottom: "2.5rem" }}>
          Tu avances prudemment dans la nuit, guidé par la forme mystérieuse. Les bruits de la forêt s’intensifient, et chaque pas te rapproche d’une découverte inattendue…
        </p>
        <button
          style={{
            background: "#E87A5D",
          ...
          onClick={() => navigate && navigate("suite-silhouette")}
        >
          Continuer
        </button>
      </div>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', borderTop: '1px solid rgba(27,67,50,0.07)', padding: '0.75rem 1.5rem 1.25rem', zIndex: 100 }}>
        <div className="nav-inner" style={{ maxWidth: 480, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <a
            ...
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, textDecoration: 'none', color: '#1B4332', fontSize: '0.72rem', fontWeight: 500, cursor: 'pointer' }}
          ...
          <a
            href="#"
            className="nav-item"
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, textDecoration: 'none', color: '#1B433258', fontSize: '0.72rem', fontWeight: 500, cursor: 'pointer' }}
            onClick={e => { e.preventDefault(); navigate && navigate('progress'); }}
            onMouseEnter={e => (e.currentTarget.style.color = '#1B4332')}
            onMouseLeave={e => (e.currentTarget.style.color = '#1B433258')}
          >
          ...
          <a
            ...
            className="nav-item"
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, textDecoration: 'none', color: '#1B433258', fontSize: '0.72rem', fontWeight: 500, cursor: 'pointer' }}
            onClick={e => { e.preventDefault(); navigate && navigate('certificate'); }}
            onMouseEnter={e => (e.currentTarget.style.color = '#1B4332')}
            onMouseLeave={e => (e.currentTarget.style.color = '#1B433258')}
          >
          ...
          <a
            ...
            className="nav-item"
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, textDecoration: 'none', color: '#1B433258', fontSize: '0.72rem', fontWeight: 500, cursor: 'pointer' }}
            onClick={e => { e.preventDefault(); navigate && navigate('profile'); }}
            onMouseEnter={e => (e.currentTarget.style.color = '#1B4332')}
            onMouseLeave={e => (e.currentTarget.style.color = '#1B433258')}
          >
